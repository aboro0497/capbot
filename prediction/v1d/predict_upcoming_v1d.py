#V2 - run if odds are missing - computes results as expected from match_upcoming - may need some fine tuning so it computes for all matches in prediction
import pandas as pd
import numpy as np
import joblib
...
# === Check odds and continue even if none are present
if df[["odds_A", "odds_B"]].isnull().all().all():
    print("⚠️ No odds found in matches_upcoming.csv — skipping prediction but continuing for result updates.")
    ...
    print("✅ Odds found — running predictions.")
    ...
    # === Debug: Confirm capbot_pick aligns with prediction probability
    print("\n🔍 CapBot Prediction Debug (Sample 10 rows):")
    sample_size = min(10, len(df))
    df_sample = df[["player_A", "player_B", "pred_proba", "capbot_pick", "odds_A", "odds_B"]].sample(sample_size, random_state=42)

    for _, row in df_sample.iterrows():
        a = row["player_A"]
        b = row["player_B"]
        proba = row["pred_proba"]
        pick = row["capbot_pick"]
        odds_a = row["odds_A"]
        odds_b = row["odds_B"]

        predicted = a if proba >= 0.5 else b
        correct = "✅" if pick == predicted else "❌"

        print(f"{correct} P(A wins): {proba:.2f} | A: {a} | B: {b} → CapBot Pick: {pick}")
...
if match_finished_path.exists():
    ...
    def clean_name(name):
        """
        Cleans player name for fuzzy matching:
        - Removes any [###] style rank
        - Removes trailing odds or junk after the rank
        - Lowercases and trims whitespace
        """
        name = str(name)
        name = re.sub(r"\[\d+\].*", "", name)  # remove [123] + any trailing characters (like odds)
        name = re.sub(r"\[\d+\]", "", name)    # also remove bare [123] just in case
        name = re.sub(r"[^a-zA-Z\s()/.-]", "", name)  # strip numbers/symbols outside names
...
# === Filtered FLOPS (from top picks only)
flops = picks[picks["status"] == "🔴 FLOP"].copy()
...
with pd.ExcelWriter(xlsx_path, engine="xlsxwriter") as writer:
    if prediction_possible:
        ...
        flops[columns_to_export].to_excel(writer, sheet_name="Filtered FLOPS", index=False)
    else:
        print("🚫 Skipping prediction sheets due to missing odds.")
