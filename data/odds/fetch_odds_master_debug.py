#working fetch odds but misses out on some name matches like
#Sultanov Khumoyun(UZB) [223] / Ramos-Vinolas Albert(ESP) [259]
#vs
#K. Sultanov / A. Ramos
...
def normalize_name(name):
    if not isinstance(name, str):
        return ""
    name = re.sub(r"\([^)]*\)", "", name)      # Remove (country)
    name = re.sub(r"\[[^]]*\]", "", name)      # Remove [rank]
...
for idx, row in upcoming_df.iterrows():
    ...
    best_match = None
    best_score = 0
    # 🔍 Filter fixture_df by date
    same_day = fixture_df[fixture_df["date"] == match_date].copy()

    # ⏰ Parse fixture times
    same_day["parsed_time"] = same_day["time"].apply(parse_time)

    # 🧪 DEBUG — Print if Sultanov is in the match
    if "sultanov" in raw_A.lower() or "sultanov" in raw_B.lower():
        print(f"\n📋 same_day fixture count: {len(same_day)}")
        print(same_day[["player_A", "player_B", "date", "time"]])

        check = fixture_df[
            (fixture_df["player_A"].str.contains("Sultanov", case=False, na=False)) |
            (fixture_df["player_B"].str.contains("Sultanov", case=False, na=False))
        ]
        print("\n🔍 ALL Sultanov Fixtures in fixture_df:")
        print(check[["player_A", "player_B", "date", "time"]])
    ...
    for _, f in same_day.iterrows():
        ...
        if score < 85:
            def to_initial_name(full):
                parts = full.split()
                if len(parts) >= 2:
                    return f"{parts[0][0]} {parts[-1]}"
                return full

            alt_A = to_initial_name(norm_A)
            alt_B = to_initial_name(norm_B)

            # Print debug pairs before scoring
            if "sultanov" in raw_A.lower() or "sultanov" in raw_B.lower():
                print(f"\n🧪 Matching:")
                print(f"  alt_A: {alt_A} vs fA: {fA}")
                print(f"  alt_B: {alt_B} vs fB: {fB}")
                print(f"  alt_A: {alt_A} vs fB: {fB}")
                print(f"  alt_B: {alt_B} vs fA: {fA}")

            initials_score1 = (partial_token_set_ratio(alt_A, fA) + partial_token_set_ratio(alt_B, fB)) / 2
            initials_score2 = (partial_token_set_ratio(alt_A, fB) + partial_token_set_ratio(alt_B, fA)) / 2
            score = max(score, initials_score1, initials_score2)

            if "sultanov" in raw_A.lower() or "sultanov" in raw_B.lower():
                print(f"🆙 Final fallback score: {round(score)}")
